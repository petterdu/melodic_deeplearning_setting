    ...
    def ObjectTracking(self, data_BoundingBoxes):
        ...
        for box in data_BoundingBoxes.bounding_boxes:
            if box.probability > 0.5:
                ...
                point = rs.rs2_deproject_pixel_to_point(self.intrinsics, [cx, cy], distance_in_m*1000)
                ...
                point_dict[object_name + str(object_count)]=[point[2],-point[0],-point[1]]
                # print(point_dict)
        ...
        max_point_x=max(point_dict.items(),key=lambda x: x[1][0])[1][0]
        max_point_y_min=min(point_dict.items(),key=lambda x: x[1][1])[1][1]
        max_point_y_max=max(point_dict.items(),key=lambda x: x[1][1])[1][1]
        max_point_z=max(point_dict.items(),key=lambda x: x[1][2])[1][2]
        # print(max_point_y_min)
        hello_float.data=[max_point_x,max_point_y_min,max_point_y_max,max_point_z]
        # print(hello_float)
        pub2.publish(hello_float)
...
if __name__ == '__main__':
    ...
    pub2 = rospy.Publisher('/point_dict', Float32MultiArray, queue_size=50) 
